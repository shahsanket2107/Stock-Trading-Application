...
/**
 * This interface extends the User Model interface and has methods to invest amounts in a portfolio
 * according to dollar cost averaging strategy. This interface also allows strategies that have no
 * end date.
 */
public interface UserModelExtension extends User {
   ...
   * @throws IllegalArgumentException if portfolio name does not exist, if date is not properly
   *                                  formatted, if the date is in the future, if amount in
   *                                  negative, if commissionFee is negative, if the sum of
   *                                  fractional distribution of stocks does not sum up to be 100%.
   */
   ...
   *
   * @param pname         is the name of portfolio to be created/ invested in according to dollar
   ...
   * @param interval      is the frequency in days at which he wants to make periodic investments in
   *                      his portfolio.
   ...
   *                                  if amount in negative, if commissionFee is negative, if the
   *                                  sum of fractional distribution of stocks does not sum up to be
   *                                  100%.
   */
   ...
   * to be invested in the future. So this method loads that strategy and compares the start date of
   * that strategy with the current date and if the start date is less than the current date, it
   ...
   *         without any error.
   * @throws IllegalArgumentException if there is some problem reading from and writing to the
   *                                  file.
   */
